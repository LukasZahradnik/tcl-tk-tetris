...
proc draw_bg {} {
    ...
    for {set i 0} {$i < [expr {($::QUEUE_LEN * 5 + 1)}]} {incr i} {
        set y [expr {$i * $::CELL_SIZE}]
...
proc main {} {
    ...
    grid [canvas .frame.ui.queue -width [expr {5 * $::CELL_SIZE}] -height [expr {($::QUEUE_LEN * 5 + 1) * $::CELL_SIZE }] -background black] -row 0 -column 0
